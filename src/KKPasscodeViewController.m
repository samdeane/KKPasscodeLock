...
- (void)viewWillAppear:(BOOL)animated
{
	...
	if (_mode == KKPasscodeModeSet || _mode == KKPasscodeModeChange) {
		if (_passcodeLockOn) {
			...
			for (NSUInteger i = 0; i < [[_boxes lastObject] count]; i++) {
	...
	for (NSUInteger i = 1; i < [_tableViews count]; i++) {
...
- (void)incrementFailedAttemptsLabel
{
    ...
    NSUInteger _failedAttemptsCount = [[KKKeychain getStringForKey:@"failedAttemptsCount"] integerValue];
...
- (BOOL)textField:(UITextField*)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString*)string
{
  ...
  for (NSUInteger i = 0; i < kPasscodeBoxesCount; i++) {
